...
class HandlerForm(Form):
    ...
    density_method = SelectField(
        'Sub-cooled density predictive technique for organic compounds', choices=[
    ...
    properties_method = SelectField(
        'Critical properties for organic compounds, if needed by density method', choices=[
            ('nannoolal',        'Nannoolal et al 2004'),
...
def handler(
        inorganic_ions, organic_compounds, interactions_method, vp_method,
        bp_method, density_method, properties_method, surface_tension,
        dry_sizes, temperatures):
    """
    Calculates the CCN activation potential an inorganic/organic mixture from a
    prescribed total condensed phase concentration, existing core and ambient
    conditions (all given *temperatures*). The *vp_method* parameter is one of
    the strings:
    ...
    *organic_compounds* and *inorganic_ions* (both given as a sequence of
    SMILES strings) at all given *temperatures* (a sequence of floating point
    values giving temperatues in degrees Kelvin).  The *density_method*
    parameter is one of the strings:
    ...
    * 'tyn_and_calus'
    ...
    (nm). Finally, the *properties_method* parameter is one of the strings:
    ...
    * 'joback_and_reid'
    ...
    for temperature in temperatures:
        ...
        for dry_size in dry_sizes:
            sat_ratio, Kappa, vp, coeffs = CCN_activation.growth_factor_model_inorg_org(
                organic_compounds, inorganic_ions, pressure_data, density_data, temperature, ideality, dry_size, surface_tension)
            for compound, result in vp.items():
                equilib_vp[(temperature,dry_size), str(compound).strip()] = result
            for compound, result in coeffs.items():
                act_coeffs[(temperature,dry_size), str(compound).strip()] = result
    ...
    return Result(
        Table(
            'totals',
            title='Critical saturation ratio (%) and Kappa value (unitless) of aerosol particle',
