...
class HandlerForm(Form):
    ...
    inorganic_ions = SMILESDictField(
        'Inorganic ions', entry_label='SMILES', data_label='µmole/m3',
        compounds=[
            ('[Na+]',             'Sodium cation'),
            ('[K+]',              'Potassium cation'),
            ('[NH4+]',            'Ammonium cation'),
    ...
    insoluble_core = CoreAbundanceField(
        'Insoluble, involatile, inert core abundnace', soluble=False
        )
...
def handler(organic_compounds, inorganic_ions, interactions_method, vp_method,
    ...
    prescribed total gaseous concentration, in existing core and ambient
    ...
    * 'evaporation'
    ...
    and Yalkowsky [MYRDAL1997]_, or evaporation [COMP2011] when calculating the
    vapour pressure. The *bp_method* parameter is one of the strings:
    ...
    * 'ideal'
    ...
    for temperature in temperatures:
        ...
        for humidity in humidities:
            water_result, compounds_result = partition_models.partition_model(
                organic_compounds, inorganic_ions, pressure_data,
